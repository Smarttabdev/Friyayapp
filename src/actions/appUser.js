...
function clearSharingToken() {
  Auth.setCookie('sharingToken', null);
}

function initClearSharingTokenHandlers() {
  window.removeEventListener('visibilitychange', clearSharingToken);
  window.removeEventListener('pagehide', clearSharingToken);
  window.addEventListener('visibilitychange', clearSharingToken);
  window.addEventListener('pagehide', clearSharingToken);
}
...
export const loginUser = ({
  ...
  try {
    const params = { user: { email, password, sharing_token } };
    const {
      data: { data }
    } = await ApiRequestWithoutToken.request({
      method: 'POST',
      url: 'sessions',
      data: params
    }).catch(({ response }) => {
    ...
    if (!user.attributes.is_guest) {
      Auth.setCookie('authToken', data.attributes.auth_token);
      Auth.setNotificationSettings(data);
      Auth.setIntegrationTokens(data);
    } else {
      Auth.setCookie('sharingToken', data.attributes.auth_token);
      initClearSharingTokenHandlers();
    }
    ...
    } else if (user_id !== null && user_email !== null) {
      isLogged = true;
      const {
        attributes: { ui_settings },
        relationships: { following_users }
      } = data;
    ...
    const { message } = err;
    !sharing_token && failure('Could not log in');
  ...
  try {
    ...
    if (user_id !== null && user_email !== null) {
      ...
      if (localStorage.getItem('user_id') === null) {
        const {
          data: { data }
        } = await ApiRequest.request({ url: 'me' });
  ...
  try {
    ...
    const changeData = {
      ...
      attributes: {
        avatar_url: get(
          data,
          'relationships.user_profile.data.avatar_url',
          avatar
        ),
        ...
        user_profile: get(
          ...
          get(
            data,
            'relationships.user_profile.data.resource_capacity',
            resourceCapacity
          )
        )
      }
...
export const updateAvtarUrl = data => dispatch => {
  dispatch(changeUser(data));
...
export const updateSettingsLocal = ({
  id,
  setting,
  value
}) => async dispatch => {
...
export const existingEmail = email => async dispatch => {
  const { data } = await ApiRequest.get('/check_email', { params: { email } });
  return data.email_presence;
};

export const finishIntroTour = () => dispatch => {
  try {
    localStorage.setItem('tour_introduction_finished', JSON.stringify(true));
  } catch (err) {
    console.error(err);
