...
export function deleteTopic(topicId, isSubtopic = false) {
  return async dispatch => {
    ...
    try {
      ...
      msg.update({
        message: isSubtopic ? 'boards deleted' : 'boards deleted',
        type: 'success',
...
export function deleteTopicAndMove(topicId, newTopicId, isSubtopic = false) {
  return async dispatch => {
    ...
    const text = isSubtopic
      ? 'Deleting boards and moving content...'
      : 'Deleting boards and moving content...';
    const msg = info(text);
    ...
    try {
      ...
      msg.update({
        message: isSubtopic
          ? 'boards deleted and content moved'
          : 'boards deleted and content moved',
        type: 'success',
        showCloseButton: true
      });
...
export function moveTopic(topicID, newTopicID) {
  return async dispatch => {
    ...
    try {
      ...
      success('boards deleted and moved');
    } catch (error) {
      handleCatch(
        error,
        dispatch,
