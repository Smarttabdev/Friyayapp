...
const setSearchTopicsResult = (state, action) => {
  const { payload } = action;
  const { searchTopicsResult } = payload;
  return { ...state, searchTopicsResult };
};

const clearSearchTopicsResult = (state, action) => {
  return { ...state, searchTopicsResult: 'ALL' };
};
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.setSearchTopicsResult:
      return setSearchTopicsResult(state, action);

    case actionTypes.clearSearchTopicsResult:
      return clearSearchTopicsResult(state, action);
