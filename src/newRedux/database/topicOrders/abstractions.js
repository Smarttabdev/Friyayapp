...
export const updateOrCreateTopicOrderFromTopicMove = ({
  ...
  const state = getState();
  let parentTopicIdOrDefault = parentTopicId || '0';
  if (parentTopicIdOrDefault == '0') {
    const rootTopic = getRootTopic(state);
    parentTopicIdOrDefault = rootTopic?.id;
  }
...
export const updateOrCreateTopicOrder = ({ topicId, subtopicOrder }) => async (
  dispatch,
  getState
) => {
  const state = getState();
...
export const updateOrCreateTopicOrderOnSheetViewColumnChange = ({
  ...
  if (updateColumnWidth) {
    return dispatch(
      updateTopicOrder({
        id: topicOrder.id,
        attributes: { sheet_view_column_width: columns }
      })
    );
  }
  const hasAsked = stateMappings(
...
const util_QueryUserForChangeOrNewOrder = ({
  ...
  vex.dialog.open({
    message: `You are currently viewing ${topicOrder.attributes.name}.  Would you like to change this order or create a new one?`,
    buttons: [
