...
class SearchInput extends Component {
  ...
  UNSAFE_componentWillReceiveProps({ multipleValues, value }) {
    if (!multipleValues) {
...
const mapState = (state, { type, value }) => {
  const sm = stateMappings(state);
  const { cards } = sm;
  const people = getPeopleObject(state);
  return {
    value: (value || []).map(val => {
      if (val.title) return val;
      switch (type) {
        case 'tips': {
          const tipId = Object.keys(cards).find(id => id == val.id);
          const title = tipId
            ? cards[tipId].attributes.title
            : `${type}:${val.id}`;

          return { ...val, title };
        }
        case 'users': {
          const user = people[val.id];
...
export default withDataManager(dataRequirements, mapState, null, {
