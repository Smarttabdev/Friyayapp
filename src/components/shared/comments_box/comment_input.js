...
class CommentInput extends Component {
  ...
  UNSAFE_componentWillReceiveProps = ({ tip }) => {
    const {
      props: { tip: oldTip }
    } = this;
  ...
  handleCommentSubmit = async e => {
    ...
    const {
      props: { tip, comment, save, update, reset },
      state: { body }
    } = this;
  ...
  toggleCommentEdit = () => {
    this.setState(state => ({
      ...state,
      isStartCommenting: !this.state.isStartCommenting
    }));
  };

  render() {
    const {
      props: { comment, isSaving, user, view },
      state: { body, isStartCommenting }
    } = this;
    return (
