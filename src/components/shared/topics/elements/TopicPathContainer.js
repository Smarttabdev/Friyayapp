...
class TopicPathContainer extends React.Component {
  ...
  async componentDidMount() {
    ...
    path.forEach(ancestor => {
      if (
        ancestor.id != topic.id &&
        (!topics[ancestor.id] || !topics[ancestor.id].relationships)
      ) {
  ...
  render() {
    const {
      props: { renderAncestor, topic, topics, rootTopic },
      state: { loading }
    } = this;
    return loading ? (
      <span>...</span>
      ...
      <Fragment>
        {get(topic, 'attributes.path', [])
          .filter(item => item.id != rootTopic.id)
          .map((ancestor, index) => renderAncestor(topics[ancestor.id], index))}
      </Fragment>
    );
  }
}
