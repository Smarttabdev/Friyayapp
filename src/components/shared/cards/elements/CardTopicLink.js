...
class CardTopicLink extends React.Component {
  ...
  render() {
    ...
    return topicToLinkTo ? (
      ...
      //       <Link className={`card-topic-link ${ style }`} key={leg.id} to={ `${baseUrl}boards/${ leg.slug }` }  >
      ...
      <Link
        ...
        to={`${baseUrl}boards/${topicToLinkTo.attributes.slug}`}
      >
        {inReference
          ? `in ${topicToLinkTo.attributes.title}`
