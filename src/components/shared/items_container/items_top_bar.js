...
const RightNavFilterOptions = {
  ...
  [FILTER.FOLLOWING]: 'Cards from Boards I Follow',
...
class ItemsTopBar extends Component {
  ...
  applyFilter = (filterType = '', selectedLabels) => {
    ...
    if (group !== null) {
      const path =
        topic !== null
          ? `/groups/${group.id}/boards/${topic.attributes.slug}`
          : `/groups/${group.id}`;
      ...
      newRoute = `/boards/${topic.attributes.slug}?filter_type=${filterType}&filter_labels=${labels}`;
...
export default connect(mapState, mapDispatch)(ItemsTopBar);
