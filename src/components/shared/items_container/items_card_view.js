...
const getSlugFromFirst = compose(getSlug, head);
const getSlugFromSecond = compose(getSlug, nth(2));
const getId = view(lensProp('id'));
const getIdFromFirst = compose(getId, head);
const getIdFromSecond = compose(getId, nth(1));
...
class ItemsCardView extends Component {
  ...
  render() {
    ...
    const sidePanelClass = classNames({
      'col-md-4 list-board side-panel': true,
      hide: !showCardList
    });
    ...
    return (
      <div className="card-board row" style={viewStyle}>
        <ItemsList
...
export default connect(mapState, mapProps)(withRouter(ItemsCardView));
