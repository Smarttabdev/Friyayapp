...
class TipActionListDropdown extends Component {
  ...
  handleItemArchiveClick(item) {
    ...
    if (handleArchiveItem) {
      isTaskView ? archiveTipInTask(item.id, topic.id) : archiveItem(item);
    } else {
      vex.dialog.confirm({
        unsafeMessage: `
  ...
  render() {
    ...
    return (
      <div
        className={classNames('dropdown', { open: isWikiView && showActions })}
      >
        {toggleLink}
        <ul
