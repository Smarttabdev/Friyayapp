...
class FileUploadBox extends Component {
  ...
  handleFilePicker = e => {
    const _this = this;
    const $fileUploadLink = !e ? null : $(e.target);
    const uploadType = !$fileUploadLink
      ? 'upload'
      : $fileUploadLink[0].dataset.type;
    ...
    if (uploadType !== 'link') {
      fileServices = ['COMPUTER', ...fileServices];
    }
  ...
  renderExternalUploadBox = () => {
    let { tabIndex, hideButtons } = this.props;
    let { fetchingExternalUploads } = this.state;

    return (
      <div
        className="form-group file-action-buttons-section"
        style={{ display: hideButtons ? 'none' : 'initial' }}
      >
        <button
          type="button"
          className="btn js-file-upload-box-button button"
...
export default connect(undefined, undefined, undefined, { withRef: true })(
  FileUploadBox
);
