...
class NotificationListing extends PureComponent {
  ...
  filterNotifications = notifications => {
    ...
    return notifications.filter(
      not =>
        not.attributes.action ==
        notificationConfig[selectedNotificationType].dbAction
    );
  };
      ...
      <div className="notification-listing">
        <div className="notification-listing_header">
          {notificationTypes.map(type => (
            <span data-tip={type.description} key={type.key}>
              <IconButton
                additionalClasses={
                  selectedNotificationType == type.key ? 'active' : ''
                }
                fontAwesome
                icon={type.icon}
                onClick={() => this.handleNotificationTypeSelect(type.key)}
              />
            </span>
          ))}
          <a
            className="btn btn-default btn-sm pull-right mark-all"
        ...
        {notificationsByAge['new'].length > 0 && (
          <Fragment>
            ...
            {this.filterNotifications(notificationsByAge['new']).map(
              notification => (
                <NotificationItem
        ...
        {notificationsByAge['yesterday'].length > 0 && (
          <Fragment>
            ...
            {this.filterNotifications(notificationsByAge['yesterday']).map(
              notification => {
                return (
                  <NotificationItem
                    notification={notification}
        ...
        {notificationsByAge['older'].length > 0 && (
          <Fragment>
            ...
            {this.filterNotifications(notificationsByAge['older']).map(
              notification => (
                <NotificationItem
                  notification={notification}
                  key={notification.id}
        ...
        {notificationsByAge.count == 0 && (
          <div className="notification-listing_header">No activity</div>
        )}
...
export const mapNotificationsToAge = createSelector(
