...
class LabelsFilter extends Component {
  ...
  handleLabelSelect = label => {
    const id = label.id;
    const {
      props: { addLabel: add, removeLabel: remove, labelsFilter }
    } = this;
  ...
  switchView = viewMode => this.setState(state => ({ ...state, viewMode }));

  render() {
    const {
      props: { isTypeLabel, labels, isLoadingLabels, labelsFilter },
      state: { viewMode }
    } = this;
    ...
    if (viewMode === 'add_label') {
      labelsFilterContent = (
        <AddLabelForm labelType={isTypeLabel} switchView={this.switchView} />
      );
    }
