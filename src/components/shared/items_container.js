...
const getPrecedingIds = index => compose(map(prop('id')), slice(0, index));
...
class ItemsContainer extends Component {
  ...
  UNSAFE_componentWillReceiveProps({ view }) {
    if (view !== BOARDS.CARD) {
      tiphive.addDetectElementScrollEnd(window);
    } else {
      tiphive.removeDetectElementScrollEnd(window);
  ...
  renderItems() {
    ...
    const itemContainerClassNames = classNames({
      ...
      'list-board': tipView === BOARDS.LIST,
      'small-grid-board': tipView === BOARDS.SMALL_GRID,
      'card-board': tipView === BOARDS.CARD,
    ...
    if (
      items.length > 0 &&
      (tipView === BOARDS.SMALL_GRID || tipView === BOARDS.GRID)
    ) {
      const emptyClass = classNames({
        'small-grid-item': tipView === BOARDS.SMALL_GRID,
        'grid-item': tipView === BOARDS.GRID
      });
  ...
  render() {
    ...
    if (!hideTipFilter && tipsView !== BOARDS.CARD) {
      itemsTopBarContent = (
        <ItemsTopBar
          group={group}
          topic={topic}
    ...
    return (
      <div className="item-container-parent">
        {itemsTopBarContent}
        {itemsPanel}
        {tipsView !== BOARDS.CARD && itemsMessage}
        {isItemPageOpen && (
...
export default connect(mapState, mapDispatch)(withRouter(ItemsContainer));
