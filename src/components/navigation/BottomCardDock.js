...
class BottomCardDock extends PureComponent {
  ...
  render() {
    const { dockCards, displayDock, cardIds } = this.props;
    ...
    return (
      displayDock && (
        <div className={`bottom-card-dock ${displayDock && 'presented'}`}>
          <div className="bottom-card-dock_items-container">{navItems}</div>
          {dropDownMenu}
          {cardIds && cardIds.length > 0 && (
            <DMLoader
              dataRequirements={{
                cardsWithAttributes: {
                  attributes: { filterIDs: cardIds }
                }
              }}
...
const dockCardsSelector = createSelector(
  [
    //recomputes every time, but returns same object if ids haven't changed, to prevent rerender
    (state, cardsInDock) =>
      cardsInDock.map(cardId => stateMappings(state).cards[cardId])
  ],
...
const mapState = (state, props) => {
  ...
  const filteredCards = sm.dock.cardsInDock.filter(
    cardId => !!sm.cards[cardId]
  );
  return {
    cardIds: sm.dock.cardsInDock,
    cards: sm.cards,
...
export default connect(mapState, mapDispatch)(BottomCardDock);
