...
const TeamGoalsLens = props => {
  ...
  const updateExpanded = (id, isExpanded) => {
    ...
    mutations.setConfig({
      ...
      config: 'TEAM_GOALS.collapsed_ids',
      value: collapsedIds
    });
  };
...
const FragmentContainer = createFragmentContainer(TeamGoalsLens, {
  query: graphql`
    fragment TeamGoalsLens_query on Query
      @argumentDefinitions(
        topicId: { type: ID }
        lenseId: { type: ID }
