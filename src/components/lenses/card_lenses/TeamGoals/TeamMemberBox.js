  ...
  return (
    <div className="team-goals-list">
      <div
        ...
        className="team-goals-list__top"
      >
        <IconButton
          fontAwesome
        ...
        <IconButton
          additionalClasses="team-goals-list__remove"
          icon="close"
      ...
      <div
        ...
        className="team-goals-list__bottom"
      >
        {selectedTopics?.length > 0
          ? selectedTopics.map(board => (
              <div key={board.id} style={{ marginBottom: '25px' }}>
                <GoalBox board={board} users={people} hideAddAssignee />
              </div>
            ))
          : null}
        ...
        <Dropdown
          trigger={
            <IconButton
              ...
              color={'#2e3037'}
              icon="add"
              tooltip={`Assign a Goal Board to ${user?.attributes?.first_name}`}
            />
          }
          closeOnClick={false}
          className="team-goals-list__add-board"
...
const RefetchContainer = createRefetchContainer(
  ...
  {
    query: graphql`
      fragment TeamMemberBox_query on Query
  ...
  graphql`
    query TeamMemberBoxRefetchQuery($owner: ID!) {
      ...TeamMemberBox_query @arguments(owner: $owner)
    }
  `
);
...
export default connect(mapState)(
  QueryRenderer(props => <RefetchContainer {...props} query={props} />, {
    query: graphql`
      query TeamMemberBoxQuery($owner: ID!) {
        ...TeamMemberBox_query @arguments(owner: $owner)
      }
    `,
    vars: ({ user }) => ({
