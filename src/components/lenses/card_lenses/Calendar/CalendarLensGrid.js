...
import CalendarLensGridRow from './CalendarLensGridRow';
import CalendarLensGridHeader from './CalendarLensGridHeader';
...
class CalendarLens extends Component {
  ...
  handleCardsScroll = ({ target }) => {
    requestAnimationFrame(() => {
      this.headerRef.style.left = `-${target.scrollLeft}px`;
    });
  };
...
export const COLUMNS_CONFIG = {
  monthDays: date => {
    const cacheKey = `${date.valueOf()}-monthDays`;
    let cacheEntry = COLUMNS_CONFIG_CACHE.get(cacheKey);
    if (cacheEntry) {
      return cacheEntry;
    }

    cacheEntry = times(date.daysInMonth()).map(index => {
      const dayOfMonth = index + 1;
      const day = date.clone().date(dayOfMonth);
      const name = day.format('dddd');
      const dayStart = day.startOf('day');
      const dayEnd = day.clone().endOf('day');
      return { id: dayOfMonth, name, dayOfMonth, range: [dayStart, dayEnd] };
    });
