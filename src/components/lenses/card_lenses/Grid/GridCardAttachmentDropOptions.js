...
class GridCardAttachmentDropOptions extends Component {
  ...
  toggleHoverClass = type =>
    this.setState(prevState => ({
      ...prevState,
      hoverHighlightClass: type ? `activate-highlight-${type}` : ''
    }));

  render() {
    const {
      props: { card, board },
      state: { hoverHighlightClass }
    } = this;

    return (
      <div className="full-width grid-attachment-options">
        {board && board.includes('new') && <p>+ Add Card</p>}

        <GenericDropZone
          dropClassName={`attachment-option${board ||
            ''} ${hoverHighlightClass.includes('link') && hoverHighlightClass}`}
          card={card}
          method="link"
          onDragEnter={() => {
            this.toggleHoverClass('link');
          }}
          onDragLeave={() => {
            this.toggleHoverClass(false);
          }}
        ...
        <GenericDropZone
          dropClassName={`attachment-option${board ||
            ''} ${hoverHighlightClass.includes('upload') &&
            hoverHighlightClass}`}
          card={card}
          method="upload"
          onDragEnter={() => {
            this.toggleHoverClass('upload');
          }}
          onDragLeave={() => {
            this.toggleHoverClass(false);
          }}
