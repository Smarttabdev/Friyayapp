...
class ItemPage extends Component {
  ...
  componentDidMount = async () => {
    const { tip, match, cardSlug } = this.props;
    document.addEventListener('keydown', this.handleKeyDown);

    if (!tip) {
      const slug = cardSlug || match.params.cardSlug;
      (async () => {
        try {
          await this.getCard(slug);
        } catch (ex) {
          console.error(ex);
        }
      })();
  ...
  handleValueUpdate = updates => {
    this.props.updateCard({ id: this.props.tip.id, ...updates });
  };
...
const mapState = (state, props) => {
  ...
  const slug = props.cardSlug || props.match.params.cardSlug;
  return {
    page: sm.page.page,
    tip: sm.cards[idFromSlug(slug)] || null,
    tips: sm.cards,
...
export default withRouter(
  withLastLocation(connect(mapState, mapDispatch)(ItemPage))
);
