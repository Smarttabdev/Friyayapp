...
class PeopleList extends Component {
  ...
  updateRole = (currentRole, newRole, userId) => {
    const {
      props: { roles, members, update, updateUserRole }
    } = this;
        ...
        .map(item => (
          <li key={`${item.id}_item`}>
            <a
              ...
              onClick={e => (
                e.preventDefault(),
                this.updateRole(currentRole, item.id, userId)
              )}
            >
              Make {item.label}
            </a>
    ...
    if (filterType !== TABS.ALL) {
      filtered = filtered.filter(member => {
        switch (filterType) {
          case TABS.ADMINS:
            return (
              member.current_role === 'admin' || member.current_role === 'me'
            );
          case TABS.MEMBERS:
            ...
            <ul className="dropdown-menu">
              ...
              <li key={`remove_item_${member.id}`}>
                <a
                  ...
                  onClick={e => (
                    e.preventDefault(),
                    this.removeMember(member.id, member.name)
                  )}
