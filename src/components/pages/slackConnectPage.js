...
class SlackConnectPage extends Component {
  ...
  renderContent() {
    ...
    const slackURL = `https://slack.com/oauth/v2/authorize?scope=${scope}&client_id=${
      window.SLACK_APP_KEY
    }&state=login/${window.currentDomainName}&redirect_uri=${
      window.currentProtocol
    }://${window.APP_HOST}${
      window.APP_PORT ? ':' + window.APP_PORT : ''
    }/slack/auth`;
