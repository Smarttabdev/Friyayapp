...
class ItemContentHeader extends PureComponent {
  ...
  handleItemArchiveClick = () => {
    const {
      props: { routerHistory, archiveItem, tip }
    } = this;
    ...
    vex.dialog.confirm({
      ...
      callback: async value => {
        if (value) {
          ...
          const nextUrl = routerHistory.location.pathname.replace(
            `/cards/${tip.id}`,
            ''
          );
  ...
  render() {
    const {
      ...
      state: { action, isMainFormOpen, isItemPageOpen }
    } = this;
    ...
    if (cardView && slug) {
      minimizeAction = (
        <a
          className="btn btn-link btn-minus"
      ...
      maximizeAction = (
        <div style={{ transform: 'rotate(270deg)' }}>
          <IconBtn
            handleClick={this.handleMaximizeClick}
            materialIcon="launch"
          />
        </div>
      );
    ...
    if (!editMode && Ability.can('update', 'self', tip)) {
      itemUpdateAction = (
        <IconBtn handleClick={handleEditClick} materialIcon="edit" />
      );
    }
    ...
    if (Ability.can('destroy', 'self', tip)) {
      itemDeleteAction = (
        <IconBtn
          handleClick={this.handleItemTrashClick}
          materialIcon="delete"
        />
      );
    }
    ...
    if (editMode) {
      itemSaveAction = (
        <IconBtn handleClick={handleSaveClick} materialIcon="save" />
      );
    }

    return (
      <div className="item-content-header flex-r-center-spacebetween">
