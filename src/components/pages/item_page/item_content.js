...
class ItemContent extends Component {
  ...
  render() {
    const {
      props: { tip, cardView, handleEditClick, history, onTopicClick, view }
    } = this;
    ...
    const {
      attributes: {
        ...
        body,
        color_index,
        attachments_json: { tip_links, documents, images, slack_links }
      }
    } = tip;
    ...
    if (tip_links && tip_links.length > 0) {
      tipLinksContent = (
        <ItemContentTipLinks item={tip} tipLinks={tip_links} showDescription />
      );
    }
