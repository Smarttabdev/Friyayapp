...
const GroupItemLabels = ({
            ...
            {labelsCategory.map((category, index) => (
              <div key={'labels-category-' + index}>
                ...
                {label_categories_hash[category.id + kind] == true ? (
                  <div className="label-category-add-label-form">
                    <LabelForm
                    ...
                    <div key={label.id}>
                      ...
                      label.relationships.label_categories.data.filter(
                        ...
                        <GroupItemLabel
                          ...
                          selected={selectedLabels.includes(label.id)}
                          label={label}
                          key={`kind-${label.attributes.kind}-group-label-category-${label.id}`}
                          switchScreen={switchScreen}
                          handleLabelChange={handleLabelChange}
                        />
                      ) : (
  ...
  return (
    <div className="list-group-menu-content" style={{ paddingBottom: 0 }}>
      {groups}
    </div>
  );
};
