...
class MainFormPage extends Component {
  ...
  onTopicSlide = ({ direction, relatedTarget }) => {
    ...
    if (direction === 'left' && slideIndex === 1 && isEmpty(topicTitle)) {
      failure('Please enter Board title');
      return false;
    }
  };
  ...
  render() {
    ...
    return (
      <PageModal
        ...
        hideModal={this.props.instantUpload}
      >
...
const dataRequirements = ({ tip, cardId }) => {
  return !tip && cardId
    ? {
        cardsWithAttributes: {
          attributes: {
...
const mapState = (state, props) => {
  ...
  const cardId = get(sm.modals, 'displayEditCardModal.cardId');
  return {
    cardId,
