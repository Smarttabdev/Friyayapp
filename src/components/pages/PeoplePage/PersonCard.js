...
const PersonCard = ({ user, viewPerson, showRole }) => (
  <div className="person-wrapper">
    ...
    />
    <a className="person-name" onClick={() => viewPerson(user.id)}>
      {user.attributes.name}
    </a>
    {showRole && (
      <p className="person-role">{user.attributes.current_domain_role}</p>
    )}
  </div>
);
...
//           <a
//             className="person-card_link"
//             onClick={() => toggleFollowUser(user.id, !userFollowsPerson)}
//           >
...
const mapState = (state, props) => {
  ...
  return {
    ...
    userFollowsPerson: state._newReduxTree.database.user.relationships.following_users.data.includes(
      props.user.id
    )
  };
};
