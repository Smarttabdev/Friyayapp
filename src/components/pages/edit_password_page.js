...
var EditPasswordPage = createClass({
  ...
  componentDidMount: function() {
    ...
    if (
      ...
      failure(
        'You should not come to this page without a reset password token.'
      );
      history.push('/login');
    }
  },
  ...
  handleSubmit: function(e) {
    ...
    var userNewPassword = $resetPasswordForm
      .find('#user_password')
      .val()
      .trim();
    var userNewPasswordConfirmation = $resetPasswordForm
    ...
    $resetPasswordXHR
      .done(function(response, status, xhr) {
        ...
        Auth.processAuthToken(function(status) {
          if (status.isLogged) {
            _this.props.history.push('/choose_domain');
          }
        });
      })
