...
const WorkspaceItem = ({ domain }) => {
  ...
  return (
    <div className="workspace-item">
      <span>
        <DomainLogo name={name} domain={domain} />
      </span>
  ...
  render() {
    ...
    const workspaceListSidebarClasses =
      !displayMenu || !isWorkspaceListSidebarOpen
        ...
        : `workspace-collection relative ${isWorkspaceListSidebarOpen &&
            'in-focus'}`;

    return (
      <div className={workspaceListSidebarClasses}>
        <h4 onClick={this.toggleWorkspaceListSidebar}>
          Workspaces{' '}
          <span>
            <Icon fontAwesome icon={'chevron-left'} />
          </span>
        </h4>
