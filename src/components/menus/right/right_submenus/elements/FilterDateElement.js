...
const FilterDateElement = ({ filter, onSetFilter, shiftKeyDown }) => {
  const handleSelectRange = ({ endDate, startDate }) => {
    ...
    const filterEnd = shiftKeyDown ? moment(endDate).endOf('week') : endDate;
...
const mapState = (state, props) => ({
  shiftKeyDown: state._newReduxTree.utilities.shiftKeyDown
});
