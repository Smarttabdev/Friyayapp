...
var Ability = {
  ...
  can: function(action, target, object) {
    ...
    if (!object || object.id == 0 || !get(object, 'relationships.abilities')) {
      //MH added object.id == 0 check to use in stage
      return true;
    }
    ...
    if (window.isSubdomain && window.currentDomain) {
      ...
      // }
      if (
        get(window, 'currentDomain.relationships.masks.data.is_owner') ||
        get(window, 'currentDomain.relationships.masks.data.is_admin')
      ) {
        return true;
      }
    }
    ...
    var objectAbilities = get(
      object,
      'relationships.abilities.data',
      object?.relationships?.abilities || {}
    );
    //
    // var isObjectOwner = objectMasks.is_owner;
    // var isObjectAdmin = objectMasks.is_admin;

    var isObjectOwner = get(
      object,
      'relationships.masks.data.is_owner',
      object?.relationships?.masks?.isOwner
    );

    var isObjectAdmin = get(
      object,
      'relationships.masks.data.is_admin',
      object?.relationships?.masks?.isAdmin
    );
    ...
    var hasPermission =
      objectAbilities[target]['can_' + action] ||
      objectAbilities[target]['can' + upperFirst(action)];
