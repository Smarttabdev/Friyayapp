...
const emitScroll = el => {
  ...
  if (
    window.elementScrollTop > window.lastElementScrollTop &&
...
const tiphive = {
  ...
  userIsGuest() {
    ...
    const isGuest =
      window.currentDomain.relationships &&
  ...
  userIsOwner() {
    if (window.currentDomain !== undefined && window.currentDomain !== '') {
      return get(
        window,
        'currentDomain.relationships.masks.data.is_owner',
        true
      );
    } else {
      return false;
  ...
  detectScrollEnd() {
    $(window).scroll(function() {
      ...
      if (
        window.windowScrollTop > window.lastWindowScrollTop &&
        window.windowScrollTop + window.windowHeight >=
          window.documentHeight - window.bottomOffset
      ) {
  ...
  detectModalScrollEnd() {
    ...
    $(selector).scroll(function() {
      ...
      if (
        window.modalScrollTop > window.lastModalScrollTop &&
        window.modalScrollTop + window.modalHeight >=
          window.documentHeight - window.bottomOffset
      ) {
  ...
  addDetectElementScrollEnd(element) {
    ...
    if (position === -1) {
      el.on('scroll.tiphive', function onScroll() {
        emitScroll(el);
      });
  ...
  detectElemScrollEnd(elemID, elemContentID, callback) {
    ...
    const lastElemScrollTop =
      localStorage.getItem('last-' + elemID + '-scroll-top') || 0;
    ...
    if (
      elemScrollTop > lastElemScrollTop &&
      elemScrollTop + elemHeight >= elemContentHeight - bottomOffset
    ) {
  ...
  fileUrl(url) {
    return url.startsWith('/') ? this.apiBase() + url : url;
  },

  apiBase() {
    return window.API_URL.replace(/(\/\/[^/]+).*/, '$1');
  },
