...
function truncateByCharacter(
  ...
  while (length > 0) {
    ...
    if (element.offsetHeight <= maximumHeight) {
      // A little bit of buffer for children ( currently only assumes activity indicator)
      element.innerHTML =
        innerHTML
          .substring(0, length - 5)
          .replace(TRAILING_WHITESPACE_REGEX, '') + ellipsisCharacter;
      indicator && element.appendChild(indicator);
      return;
    }
    length--;
  }
...
export default function(element, { lineCount, ellipsisCharacter } = {}) {
  ...
  const indicator = element.children[0];
