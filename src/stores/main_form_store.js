      ...
      .fail(function(xhr, status, error) {
        ...
        if (detail && detail.resourceUrl) {
          ...
          APIRequest.showInfoMessage(
            'That Board already exists. Redirecting to the existing Board.'
          );
          _this.props.history.push('/' + detail.resourceUrl);
        }
      });
  },
  ...
  updateTopic: function(
    ...
    $updateXHR
      .done(function(response, status, xhr) {
        ...
        APIRequest.showSuccessMessage('board updated');
      })
      .fail(function(xhr, status, error) {
    ...
    APIRequest.delete({
      ...
      .done(() => {
        ...
        APIRequest.showSuccessMessage(
          isSubtopic ? 'board deleted' : 'board deleted'
        );
      })
      .fail(xhr => {
        APIRequest.showErrorMessage(xhr.responseJSON.errors.detail);
  ...
  moveTopic: function(topicID, newTopicID, newTopicSlug) {
    ...
    $moveXHR
      .done(function(response, status, xhr) {
        ...
        APIRequest.showSuccessMessage('board moved');
      })
      .fail(function(xhr, status, error) {
        APIRequest.showErrorMessage(xhr.responseJSON.errors.detail);
      });
