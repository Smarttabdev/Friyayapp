...
const initialState = {
  ...
  introTourFinished:
    JSON.parse(localStorage.getItem('tour_introduction_finished')) || false
};
...
const setUser = (state, payload) => {
  const {
    id,
    attributes: { email, username, first_name, last_name, trello_token },
  ...
  if (!isEqual(state.username, username))
    localStorage.setItem('user_username', username);
  if (!isEqual(state.name, name)) localStorage.setItem('user_name', name);
  if (!isEqual(state.avatar, avatar))
    localStorage.setItem('user_avatar', avatar);
  if (!isEqual(state.firstName, first_name))
    localStorage.setItem('user_first_name', first_name);
  if (!isEqual(state.lastName, last_name))
